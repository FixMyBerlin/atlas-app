...
import { testFilterConfig, testFilterData } from './mergeFilterDataWithConfig.test'
...
const testLayerFilterWithAll = ['all', testLayerFilterSimple, testLayerFilterSimple2]
...
describe('specifyFilters()', () => {
  describe('layer filter BUT no data filter', () => {
    ...
    test('With "all" layer filter', () => {
      const filter = specifyFilters(testLayerFilterWithAll, undefined, undefined)
  ...
  describe('layer filter AND data filter', () => {
    test('layer filter is simple (no "all")', () => {
      const filter = specifyFilters(testLayerFilterSimple, testFilterData, testFilterConfig)
    ...
    test('layer filter has "all"', () => {
      const filter = specifyFilters(testLayerFilterWithAll, testFilterData, testFilterConfig)
