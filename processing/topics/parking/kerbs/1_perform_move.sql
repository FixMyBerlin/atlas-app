...
SELECT osm_type, osm_id, id, tags, meta, ST_OffsetCurve (
    ST_Simplify (geom, 0.5),
    (tags ->> 'perform_move')::numeric
  ) as geom, minzoom INTO parking_kerbs_moved
   FROM parking_kerbs WHERE
   st_issimple(geom) and st_isvalid(geom) and not st_isclosed(geom);
