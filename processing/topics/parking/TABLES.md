...
flowchart TB
    subgraph sql ["osm2pgsql"]
        ...
        subgraph sg_table ["source data"]
            nn -->|"osm2gpsql"| SGOA("<code>parking_source_germany_<br>obstacle_areas</code>")
            nn -->|"osm2gpsql"| SGOP("<code>parking_source_germany_<br>obstacle_points</code>")
    ...
    subgraph sql ["SQL"]
        subgraph init_step ["working data"]
            SGOA -->|"bbox copy"| SROA("<code>parking_source_regions_<br>obstacle_areas</code>")
            SGOP -->|"bbox copy"| SROP("<code>parking_source_regions_<br>obstacle_points</code>")
            SGR -->|"bbox copy"| SRR("<code>parking_source_regions_<br>roads</code>")
