...
from constants import available_datasets, function_table_mapping, verification_tables
import psycopg2
import json
import os
...
def create_verification_tables():
  ...
  with conn.cursor() as cursor:
    ...
    for original_table, table_name in verification_tables.items():
      print('Create verification table ', table_name)
      processed_sql = sql.replace('{table_name}', table_name)
      cursor.execute(processed_sql)
