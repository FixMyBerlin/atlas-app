...
-- move the geometry by `__offset` (+ left / - right)
...
-- We also need a unique osm_id for our frontend code. As a workaround we use the _offset sign.
-- In turn this brakes updating tables via osm2pgsql
UPDATE
  "bikelanes"
SET
  geom = ST_Transform(ST_OffsetCurve(ST_Simplify(ST_Transform(geom, 25833), 0.5), (tags->>'_offset')::numeric), 3857),
WHERE
  ST_IsSimple(geom)
  AND NOT ST_IsClosed(geom)
  AND (tags->>'_offset')::numeric != 0;
...
WHERE
  (tags->>'_offset')::numeric > 0;

-- ALTER TABLE "bikelanes" DROP COLUMN "__offset";
